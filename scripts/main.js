...
function headerMobile() {
  ...
  const tl = gsap.timeline({ paused: true, defaults: { ease: "expo.inOut", duration: 1 } });
  ...
  cards.forEach(card => {
    ...
    card.addEventListener("click", function () {
      const state = Flip.getState(
        [cardInner, cardIntro, cardExpanded], { props: "backgroundColor,color,opacity" }
      );
...
function footerMobile() {
  ...
  const footerListSolutions = document.querySelector(".c-footer-list.solutions");
  const footerListExpertise = document.querySelector(".c-footer-list.expertise");
...
function footerDesktop() {
  ...
  const footerListExpertise = document.querySelector(".c-footer-list.expertise");
  const footerListCompany = document.querySelector(".c-footer-list.company");
  const originalLocation = document.querySelector(".c-footer-list-wrap.company-expertise-location");
...
function glossaryAccordions() {
  ...
  newAccordions.forEach((accordion) => {
    ...
    gsap.set(arrow, { rotation: 0, background: "transparent", color: "white" });
  ...
  window.fsAttributes.push([
    ...
    filterInstances => {
      ...
      filterInstance.listInstance.on('renderitems', (renderedItems) => {
        ...
        const glossaryItem = document.querySelectorAll(".c-ac-item-glossary")[0];
...
function searchBar() {
  ...
  const searchPageInput = document.querySelector(".c-glossary-search.global-search");
...
function homeLoader() {
  ...
  gsap.to([heroTopText, heroBottomText], { yPercent: 0, opacity: 1, stagger: 0.2 }, "<1");
...
function solutionLoader() {
  ...
  tl.fromTo(image, { clipPath: "inset(50% 50% 50% 50%)" }, { clipPath: "inset(0% 0% 0% 0%)" });
...
mm.add("(max-width: 991px)", () => {
  footerMobile();
  headerMobile();
  document.querySelector(".c-header_lt").setAttribute("data-lenis-prevent", "");
  return () => {
    footerDesktop();
    ScrollTrigger.refresh();
    document.querySelector(".c-header_lt").removeAttribute("data-lenis-prevent", "");
  };
});
